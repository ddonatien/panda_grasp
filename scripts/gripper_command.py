...
class GripperCommander(object):
    ...
    def __init__(self, robot, jointStatePublisher, jointPrefix, armName, goal_time_tolerance=None):
        self.jointNames = [
            armName + '_finger_joint1'
        ]
        self.motorNames = [
            armName + '_finger_joint1',
            armName + '_finger_joint2'
        ]
        self.robot = robot
        ...
        self.server = actionlib.ActionServer(armName + "_hand_controller/follow_joint_trajectory",
    ...
    def on_goal(self, goal_handle):
        ...
        if set(goal_handle.get_goal().trajectory.joint_names) != set(self.prefixedJointNames):
            rospy.logerr("%s Hand controller : Received a goal with incorrect joint names: (%s)" %
                         (self.armName, ', '.join(goal_handle.get_goal().trajectory.joint_names)))
            rospy.logerr("%s Hand controller : Available joints: (%s)" %
                         (self.armName, ', '.join(self.prefixedJointNames)))
    ...
    def on_cancel(self, goal_handle):
        ...
        if goal_handle == self.goal_handle:
            ...
            for i in range(len(self.motorNames)):
                self.motors[i].setPosition(self.sensors[0].getValue())
    ...
    def update(self):
        if self.robot and self.trajectory:
            ...
            if (now - self.trajectory_t0) <= self.trajectory.points[-1].time_from_start.to_sec():  # Sending intermediate points
                ...
                for i in range(len(self.motorNames)):
                    self.motors[i].setPosition(setpoint.positions[0])
